...
from utility.models import Message, Schedule
from utils.response import *
from web.serializers import ScheduleSerializer
...
class ScheduleApis(viewsets.ModelViewSet):
    ...
    def forbid(self, request, *args, **kwargs):
        ...
        if stat == settings.SCHEDULE_FORBIDDEN_FALSE:
            for obj in objs:
                forbid = Message.objects.filter(target_users__id=obj.owner_id, related_schedule_id=obj.id,
                                                type=settings.MESSAGE_TYPE_SCHEDULE_FORBIDDEN)
                if forbid:
                    ...
                    cancel = Message.objects.filter(target_users__id=obj.owner_id, related_schedule_id=obj.id,
                                                    type=settings.MESSAGE_TYPE_SCHEDULE_FORBIDDEN_CANCEL)
                    if not cancel:
                        Message.create_message(None, obj.owner, type=settings.MESSAGE_TYPE_SCHEDULE_FORBIDDEN_CANCEL,
                                               schedule=obj)
        else:
            for obj in objs:
                cancel = Message.objects.filter(target_users__id=obj.owner_id, related_schedule_id=obj.id,
                ...
                if cancel:
                    ...
                    forbid = Message.objects.filter(target_users__id=obj.owner_id, related_schedule_id=obj.id,
                                                    type=settings.MESSAGE_TYPE_SCHEDULE_FORBIDDEN)
                    if not forbid:
                        Message.create_message(None, obj.owner, type=settings.MESSAGE_TYPE_SCHEDULE_FORBIDDEN,
                                               schedule=obj)
