...
from user.models import TravelNotes
...
class NLPModel:
    ...
    def my_init(self):
        ...
        if self.last_update is None:
            travels = list(TravelNotes.objects.all().filter(Q(forbidden=settings.TRAVEL_FORBIDDEN_FALSE,
                                                              visibility=settings.TRAVEL_VISIBILITIES_ALL)).values_list(
                'id',
                'content'))
        else:
            travels = list(TravelNotes.objects.all().filter(Q(forbidden=settings.TRAVEL_FORBIDDEN_FALSE,
                                                              visibility=settings.TRAVEL_VISIBILITIES_ALL) & Q(
                time__gt=self.last_update)).values_list('id', 'content'))
        strs = dict()
