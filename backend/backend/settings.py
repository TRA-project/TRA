...
Generated by 'django-manager startproject' using Django 2.2.3.
...
CRONJOBS = (
    ...
    #  '2>&1'每项工作执行后要做的事
    # 每小时进行一次日程通知
    ('00 */1 * * *', 'app.timedMessage.schedule_notify', '>>' + CRONJOBS_DIR + CRONJOBS_FILE_NAME + ' 2>&1'),  # 每整点执行一次
    # 每天十点进行一次航班通知
    ('00 22 * * *', 'app.timedMessage.flight_notify', '>>' + CRONJOBS_DIR + CRONJOBS_FILE_NAME + ' 2>&1'),  # 每天22点执行
    # * * * * *
