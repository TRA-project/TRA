...
from .models import Schedule, Message, UsersFlight, ScheduleItem
import datetime
...
def schedule_notify():
    try:
        ...
        scheduleitems = ScheduleItem.objects.all().filter(
        ...
        for item in scheduleitems:
            Message.create_message(source_user=None, target_user=item.schedule.owner,
                                   ...
                                   scheduleitem=item)
        print(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + " " + str(scheduleitems.count()) + " 项日程已通知")
