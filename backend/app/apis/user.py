...
from app.models import AppUser, Travel, Companion, Image, Message, \
    TravelCollection, ScheduleItem, Schedule, UsersFlight, Address
from app.serializers import UserSerializer, TravelSerializer, MessageSerializer, \
    CompanionSerializer, LogSerializer, ScheduleItemSerializer, FlightSerializer, UsersFlightSerializer
from app.utilities import permission, wechat
from app.response import *
from utilities import conversion, date, encrypt, permission as _permission, filters
from django.core.files.uploadedfile import UploadedFile

from . import tools
from .travel import TravelFilterBackend
from utilities import mixins as _mixins
...
class UserApis(viewsets.GenericViewSet, mixins.CreateModelMixin,
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Log
        tools.save_log(user_id=obj.id, action=settings.LOG_USER_REGISTER, target_id=None)
    ...
    def _retrieve(self, user, detail=False, request_user_id=None):
        ...
        data = serializer.data
        likes = Travel.objects.filter(owner=user).annotate(likes_count=Count('likes')).aggregate(Sum('likes_count'))[
            'likes_count__sum']
        if likes is None:
            likes = 0
        data['likes'] = likes
    ...
    def login(self, request, *args, **kwargs):
        ...
        if pw_sha256 == real_pw:
            ...
            # Log
            tools.save_log(user_id=user.id, action=settings.LOG_USER_LOGIN, target_id=None)
    ...
    def logout(self, request, *args, **kwargs):
        uid = _permission.user_check(request)
    ...
    def history(self, request, *args, **kwargs):
        ...
        if not user:
            return error_response(Error.INVALID_USER, status=status.HTTP_403_FORBIDDEN)
        travels = Travel.objects.filter(owner_id=request_user)
