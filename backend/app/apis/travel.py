...
from django.db.models import Sum, F, Count, Q, Max, DurationField, DateTimeField, IntegerField, ExpressionWrapper, \
    BooleanField, Exists, Subquery, OuterRef
...
from app.models import Travel, AppUser, Image, Address, Comment, Message, TravelCollection, Tag, Schedule, \
    ShieldingWords
...
class TravelApis(viewsets.ModelViewSet):
    ...
    def similar(self, request, *args, **kwargs):
        ...
        try:
            ...
            similar_ids.pop(-1)
        queryset = Travel.objects.all().filter(Q(id__in=similar_ids) &
