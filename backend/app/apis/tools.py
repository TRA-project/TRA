...
from app.models import Schedule, ScheduleItem, AppUser, Travel, Log
from django.conf import settings
from app.serializers import ScheduleSerializer, ScheduleItemSerializer
...
def get_recommend_cities(user_id):
    ...
    heat_cities = Travel.objects.all().filter(Q(forbidden=settings.TRAVEL_FORBIDDEN_FALSE,
    ...
    view_cities = Travel.objects.all().filter(Q(forbidden=settings.TRAVEL_FORBIDDEN_FALSE,
                                                visibility=settings.TRAVEL_VISIBILITIES_ALL) &
...
def merge_scheduleitems(src_sch, dst_sch):
    serializer = ScheduleSerializer(src_sch)
    data = serializer.data
    schedule_items = data.pop('schedule_items', None)
    for item in schedule_items:
