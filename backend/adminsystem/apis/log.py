...
from app.models import Message, AppUser, Flight, UsersFlight, City, Province, Log, Travel, Companion
from adminsystem.serializers import LogSerializer
...
class LogApis(viewsets.GenericViewSet, viewsets.mixins.RetrieveModelMixin,
    ...
    def statistics(self, request, *args, **kwargs):
        ...
        for i in range(6, -1, -1):
            ...
            travel_count = Travel.objects.all().filter(Q(time__gte=dtime_bf)
        ...
        travel_area = Travel.objects.all().exclude(position=None).values(province=F('position__province')). \
    ...
    def userface(self, request, *args, **kwargs):
        ...
        # count tags
        create_travels = Travel.objects.all().filter(owner_id=owner_id).values('tag').annotate(
        ...
        view_travels = Travel.objects.all().filter(id__in=view_list).values('tag').annotate(
