...
from app.models import Companion, Message
from app.response import *
from adminsystem.serializers import CompanionSerializer
from utilities import conversion, permission as _permission, filters, date, mixins
from app.apis.companion import CompanionFilterBackend
...
class CompanionApis(viewsets.ModelViewSet):
    ...
    def forbid(self, request, *args, **kwargs):
        ...
        if stat == settings.TRAVEL_FORBIDDEN_FALSE:
            for obj in objs:
                forbid = Message.objects.filter(target_users__id=obj.owner_id, related_companion_id=obj.id, type=settings.MESSAGE_TYPE_COMPANION_FORBIDDEN)
                if forbid:
                    ...
                    cancel = Message.objects.filter(target_users__id=obj.owner_id, related_companion_id=obj.id, type=settings.MESSAGE_TYPE_COMPANION_FORBIDDEN_CANCEL)
                    if not cancel:
                        Message.create_message(None, obj.owner, type=settings.MESSAGE_TYPE_COMPANION_FORBIDDEN_CANCEL, companion=obj)
        else:
            for obj in objs:
                cancel = Message.objects.filter(target_users__id=obj.owner_id, related_companion_id=obj.id, type=settings.MESSAGE_TYPE_COMPANION_FORBIDDEN_CANCEL)
                if cancel:
                    ...
                    forbid = Message.objects.filter(target_users__id=obj.owner_id, related_companion_id=obj.id, type=settings.MESSAGE_TYPE_COMPANION_FORBIDDEN)
                    if not forbid:
                        Message.create_message(None, obj.owner, type=settings.MESSAGE_TYPE_COMPANION_FORBIDDEN, companion=obj)
