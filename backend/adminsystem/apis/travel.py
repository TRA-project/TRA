...
from app.models import Travel, Image, Message
from adminsystem.serializers import TravelSerializer
from app.response import response
from utilities.conversion import get_list
from app.response import *
from utilities import conversion, permission as _permission, filters, date
...
class TravelApis(viewsets.ModelViewSet):
    filter_backends = [TravelFilterBackend]
    ...
    def forbid(self, request, *args, **kwargs):
        ...
        objs = Travel.objects.filter(id__in=ids)
        query = {
            'forbidden': stat,
        }
        ...
        if stat == settings.TRAVEL_FORBIDDEN_FALSE:
            for obj in objs:
                forbid = Message.objects.filter(target_users__id=obj.owner_id, related_travel_id=obj.id, type=settings.MESSAGE_TYPE_TRAVEL_FORBIDDEN)
                if forbid:
                    ...
                    cancel = Message.objects.filter(target_users__id=obj.owner_id, related_travel_id=obj.id, type=settings.MESSAGE_TYPE_TRAVEL_FORBIDDEN_CANCEL)
                    if not cancel:
                        Message.create_message(None, obj.owner, type=settings.MESSAGE_TYPE_TRAVEL_FORBIDDEN_CANCEL, travel=obj)
        else:
            for obj in objs:
                cancel = Message.objects.filter(target_users__id=obj.owner_id, related_travel_id=obj.id, type=settings.MESSAGE_TYPE_TRAVEL_FORBIDDEN_CANCEL)
                if cancel:
